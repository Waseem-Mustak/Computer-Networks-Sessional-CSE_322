...
class RoutingExperiment
{
  ...
  private:
    ...
    // New member variables for the command-line arguments
    int m_nNodes{0};            //!< Number of nodes, default 0
    int m_packetsPerSec{0};    //!< Packets per second, default 0
    int m_nodeSpeed{0};          //!< Node speed, default 0 m/s
};
...
RoutingExperiment::CommandSetup(int argc, char** argv)
{
    ...
    cmd.AddValue("nNodes", "Number of nodes", m_nNodes);  // New argument for number of nodes
    cmd.AddValue("packetsPerSec", "Number of packets per second", m_packetsPerSec);  // New argument for packets/sec
    cmd.AddValue("nodeSpeed", "Node speed in m/s", m_nodeSpeed);  // New argument for node speed

    cmd.Parse(argc, argv);

    std::vector<std::string> allowedProtocols{"OLSR", "AODV", "DSDV", "DSR"};

    if (std::find(std::begin(allowedProtocols), std::end(allowedProtocols), m_protocolName) == std::end(allowedProtocols))
    {
        NS_FATAL_ERROR("No such protocol: " << m_protocolName);
    }

    // Print out the values of the new parameters for confirmation
    // std::cout << "Number of Nodes: " << m_nNodes << std::endl;
    // std::cout << "Packets per second: " << m_packetsPerSec << std::endl;
    // std::cout << "Node speed: " << m_nodeSpeed << " m/s" << std::endl;
}
...
RoutingExperiment::Run()
{
    ...
    std::string rate = std::to_string(m_packetsPerSec*64*8) + "bps";
    ...
    if (m_flowMonitor)
    {
        ...
        // Retrieve flow stats and calculate throughput
        flowmon->CheckForLostPackets(); // Optional: check for packet loss
        auto stats = flowmon->GetFlowStats(); // Get the flow statistics
        double totalBytesReceived = 0.0;
        double totalSimulationTime = TotalTime; // Simulation duration in seconds

        double totalDelay = 0.0;
        uint32_t totalPacketsReceived = 0;
        uint32_t totalPacketsSent = 0; // Added to calculate PDR

        for (auto const &flow : stats)
        {
            totalBytesReceived += flow.second.rxBytes; // Sum up all received bytes

            // Delay calculation
            totalDelay += flow.second.delaySum.GetSeconds();
            totalPacketsReceived += flow.second.rxPackets;
            totalPacketsSent += flow.second.txPackets; // Sum up all transmitted packets
        }


        

        double throughput = (totalBytesReceived * 8) / totalSimulationTime; // Convert to bits per second
        NS_LOG_UNCOND("Total Network Throughput: " << throughput << " bits per second");

        double averageDelay = totalPacketsReceived > 0 ? (totalDelay / totalPacketsReceived) : 0.0;
        NS_LOG_UNCOND("Average End-to-End Delay: " << averageDelay << " seconds");

        // Packet delivery ratio calculation
        double packetDeliveryRatio = totalPacketsSent > 0
                                        ? (static_cast<double>(totalPacketsReceived) / totalPacketsSent) * 100
                                        : 0.0;
        NS_LOG_UNCOND("Packet Delivery Ratio: " << packetDeliveryRatio << " %");

        // Packet Drop Ratio calculation
        double packetDropRatio = totalPacketsSent > 0 ? 
            (static_cast<double>(totalPacketsSent - totalPacketsReceived) / totalPacketsSent) * 100 : 0.0;
        NS_LOG_UNCOND("Packet Drop Ratio: " << packetDropRatio << " %");


//////////////////////////////////
        std::ofstream metricsFile("task1.csv", std::ios::app);
        metricsFile << nWifis << "," << m_packetsPerSec << "," << m_nodeSpeed << ","
                    << throughput << "," << averageDelay << "," << packetDeliveryRatio << "," << packetDropRatio << "\n";
        metricsFile.close();
