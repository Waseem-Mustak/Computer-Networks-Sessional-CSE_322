...
RoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
    ...
    if (IsMyOwnAddress(rreqHeader.GetDst()))
    {
        ...
        // revReq(rreqHeader);
        Rev_SendReply(rreqHeader, toOrigin);
        return;
    }
    /*
...
void RoutingProtocol::Rev_SendReply(const RreqHeader& rreqHeader, const RoutingTableEntry& toOrigin)
{
    ...
    {
        m_seqNo++;  // Increment the sequence number if valid
    }

    // Create the Rev_RrepHeader once
    Rev_RreqHeader revRreqHeader(
        /*m_flags=*/0,
        /*m_reserved=*/0,
        /*m_hopCount=*/0,
        /*m_requestID=*/rreqHeader.GetId(),            // Use the ID from RREQ
        /*m_dst=*/rreqHeader.GetDst(),              // Destination is the origin from RREQ
        /*m_dstSeqNo=*/m_seqNo,                        // Sequence number is our updated sequence number
        /*m_origin=*/rreqHeader.GetOrigin(),              // Origin is the destination from RREQ
        /*m_originSeqNo=*/0     // Use the original destination sequence number
    );

    // Create the TypeHeader for reverse RREP
    TypeHeader tHeader(AODVTYPE_RRREQ);  // Set type header for reverse RREP

    // Loop through all interfaces to send broadcast
    for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
    {
        Ptr<Socket> socket = j->first;
        Ipv4InterfaceAddress iface = j->second;

        // Create the packet for the RREP
        Ptr<Packet> packet = Create<Packet>();
        SocketIpTtlTag tag;
        tag.SetTtl(toOrigin.GetHop());  // Set TTL to the hop count of the original destination's route
        packet->AddPacketTag(tag);
        packet->AddHeader(revRreqHeader);  // Add the Rev_RrepHeader
        packet->AddHeader(tHeader);       // Add the TypeHeader for reverse RREP

        // Broadcast the packet to the appropriate destination
        Ipv4Address destination;
        if (iface.GetMask() == Ipv4Mask::GetOnes())  // If the mask is /32 (single host network)
        {
            destination = Ipv4Address("255.255.255.255");  // Broadcast to all
        }
        else
        {
            destination = iface.GetBroadcast();  // Subnet-directed broadcast
        }

        NS_LOG_DEBUG("Broadcasting Rev_RREP with id " << revRreqHeader.GetId() << " to " << destination);
...
// void
// RoutingProtocol::revReq_2(const RreqHeader &rreqHeader)
// {
...
//     }
//     // Create RRREQ header
//     Rev_RreqHeader Rev_rreqHeader;
//     Rev_rreqHeader.SetDst(dst);
...
//         if (rt.GetValidSeqNo())
//         {
//             Rev_rreqHeader.SetDstSeqno(rt.GetSeqNo());
//         }
//         else
//         {
//             Rev_rreqHeader.SetUnknownSeqno(true);
//         }
//         rt.SetHop(ttl);
//         rt.SetFlag(IN_SEARCH);
//         rt.SetLifeTime(m_pathDiscoveryTime);
...
//     if (m_gratuitousReply)
//     {
//         Rev_rreqHeader.SetGratuitousRrep(true);
//     }
//     if (m_destinationOnly)
//     {
//         Rev_rreqHeader.SetDestinationOnly(true);
//     }

//     m_seqNo++;
//     Rev_rreqHeader.SetOriginSeqno(m_seqNo);
//     m_requestId++;
//     Rev_rreqHeader.SetId(m_requestId);
...
//         Rev_rreqHeader.SetOrigin(iface.GetLocal());
//         m_rreqIdCache.IsDuplicate(iface.GetLocal(), m_requestId);
...
//         tag.SetTtl(ttl);
//         packet->AddPacketTag(tag);
//         packet->AddHeader(Rev_rreqHeader);
//         TypeHeader tHeader(AODVTYPE_RREQ);
//         packet->AddHeader(tHeader);
//         // Send to all-hosts broadcast if on /32 addr, subnet-directed otherwise
//         Ipv4Address destination;
...
//             destination = iface.GetBroadcast();
//         }
//         NS_LOG_DEBUG("Send RREQ with id " << Rev_rreqHeader.GetId() << " to socket");
//         m_lastBcastTime = Simulator::Now();
//         Simulator::Schedule(Time(MilliSeconds(m_uniformRandomVariable->GetInteger(0, 10))),
//                             &RoutingProtocol::SendTo,
//                             this,
...
// void
// RoutingProtocol::revReq(const RreqHeader &rreqHeader)
// {
//     NS_LOG_FUNCTION(this);

//     // Create a Rev_RreqHeader from the RreqHeader
//     Rev_RreqHeader revRreqHeader;
//     revRreqHeader.SetOrigin(rreqHeader.GetDst());            // Swap destination as origin
//     revRreqHeader.SetDst(rreqHeader.GetOrigin());            // Swap origin as destination
//     revRreqHeader.SetOriginSeqno(rreqHeader.GetDstSeqno());  // Set sequence number
//     revRreqHeader.SetHopCount(0);                            // Reset hop count for reverse RREQ

//     NS_LOG_DEBUG("Constructed Rev_RreqHeader: Origin=" << revRreqHeader.GetOrigin()
//                                                        << ", Destination=" << revRreqHeader.GetDst()
//                                                        << ", SeqNo=" << revRreqHeader.GetOriginSeqno()
//                                                        << ", HopCount=" << static_cast<uint32_t>(revRreqHeader.GetHopCount()));

//     // Broadcast the Rev_RreqHeader
//     for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
//     {
//         Ptr<Socket> socket = j->first;
//         Ipv4InterfaceAddress iface = j->second;
...
//         TypeHeader tHeader(AODVTYPE_RRREQ);  // Corrected type for reverse request
//         packet->AddHeader(tHeader);

//         Ipv4Address broadcastAddr = (iface.GetMask() == Ipv4Mask::GetOnes())
//                                         ? Ipv4Address("255.255.255.255")
//                                         : iface.GetBroadcast();

//         NS_LOG_DEBUG("Broadcasting Rev RREQ to " << broadcastAddr);
...
//     // A node ignores the Rev_RREQ if it has received a reverse RREQ from a node in its blacklist
//     RoutingTableEntry toPrev;
//     if (m_routingTable.LookupRoute(src, toPrev))
//     {
//         if (toPrev.IsUnidirectional())
//         {
//             NS_LOG_DEBUG("Ignoring Rev_RREQ from node in blacklist");
//             return;
//         }
//     }

//     // Check for duplicate reverse RREQ using the ID from the header
//     uint32_t id = revRreqHeader.GetId();
//     Ipv4Address origin = revRreqHeader.GetOrigin();

//     if (m_rreqIdCache.IsDuplicate(origin, id))
//     {
//         NS_LOG_DEBUG("Ignoring Rev_RREQ due to duplicate");
//         return;
//     }

//     // Increment Hop Count for the reverse RREQ
//     uint8_t hop = revRreqHeader.GetHopCount() + 1;
//     revRreqHeader.SetHopCount(hop);

//     // Check the routing table for the origin (destination of the RREQ)
//     RoutingTableEntry toOrigin;
//     if (!m_routingTable.LookupRoute(origin, toOrigin))
//     {
//         // If no route exists, create a reverse route
//         Ptr<NetDevice> dev = m_ipv4->GetNetDevice(m_ipv4->GetInterfaceForAddress(receiver));
//         RoutingTableEntry newEntry(
//             dev,               // Device
//             origin,            // Destination (origin of the reverse request)
//             true,              // Valid sequence number
//             revRreqHeader.GetOriginSeqno(), // Sequence number
//             m_ipv4->GetAddress(m_ipv4->GetInterfaceForAddress(receiver), 0), // Interface address
//             hop,               // Hop count
//             src,               // Next hop (the node from which the Rev_RREQ was received)
//             Time(2 * m_netTraversalTime - 2 * hop * m_nodeTraversalTime) // Lifetime
//         );
//         m_routingTable.AddRoute(newEntry);
//     }
//     else
...
//         if (toOrigin.GetValidSeqNo())
//         {
//             if (int32_t(revRreqHeader.GetOriginSeqno()) - int32_t(toOrigin.GetSeqNo()) > 0)
//             {
//                 toOrigin.SetSeqNo(revRreqHeader.GetOriginSeqno());
//             }
//         }
//         else
//         {
...
//     // If the source is found in the routing table, update the entry for the neighbor
//     RoutingTableEntry toNeighbor;
//     if (!m_routingTable.LookupRoute(src, toNeighbor))
//     {
//         // Create a new neighbor entry if not found
//         NS_LOG_DEBUG("Neighbor " << src << " not found in routing table. Creating an entry");
//         Ptr<NetDevice> dev = m_ipv4->GetNetDevice(m_ipv4->GetInterfaceForAddress(receiver));
//         RoutingTableEntry newEntry(dev,
//                                    src,
//                                    false,                 // Invalid seq number
//                                    revRreqHeader.GetOriginSeqno(),
//                                    m_ipv4->GetAddress(m_ipv4->GetInterfaceForAddress(receiver), 0),
//                                    1,                     // 1 hop count to source
//                                    src,                   // Next hop is the source
//                                    m_activeRouteTimeout); // Route timeout
//         m_routingTable.AddRoute(newEntry);
//     }
//     else
//     {
//         // Update the neighbor's route information
//         toNeighbor.SetLifeTime(m_activeRouteTimeout);
//         toNeighbor.SetValidSeqNo(false);
//         toNeighbor.SetSeqNo(revRreqHeader.GetOriginSeqno());
//         toNeighbor.SetFlag(VALID);
//         toNeighbor.SetOutputDevice(m_ipv4->GetNetDevice(m_ipv4->GetInterfaceForAddress(receiver)));
//         toNeighbor.SetInterface(m_ipv4->GetAddress(m_ipv4->GetInterfaceForAddress(receiver), 0));
//         toNeighbor.SetHop(1);
...
//     // If the receiver is the destination, generate a RREP
//     if (IsMyOwnAddress(revRreqHeader.GetDst()))
//     {
//         m_routingTable.LookupRoute(origin, toOrigin);
//         NS_LOG_DEBUG("Send reverse reply since I am the destination");
//         // revReq(revRreqHeader); // Broadcast reverse RREP
//         return;
//     }

//     // Otherwise, forward the reverse RREQ if conditions match
//     RoutingTableEntry toDst;
...
//         if (toDst.GetNextHop() == src)
//         {
//             NS_LOG_DEBUG("Drop Rev_RREQ from " << src << ", dest next hop " << toDst.GetNextHop());
//             return;
//         }

//         if ((revRreqHeader.GetUnknownSeqno() ||
//              (int32_t(toDst.GetSeqNo()) - int32_t(revRreqHeader.GetDstSeqno()) >= 0)) &&
//             toDst.GetValidSeqNo())
//         {
//             if (!revRreqHeader.GetDestinationOnly() && toDst.GetFlag() == VALID)
//             {
//                 m_routingTable.LookupRoute(origin, toOrigin);
//                 SendReplyByIntermediateNode(toDst, toOrigin, revRreqHeader.GetGratuitousRrep());
//                 return;
//             }
//             revRreqHeader.SetDstSeqno(toDst.GetSeqNo());
//             revRreqHeader.SetUnknownSeqno(false);
//         }
//     }

//     // Broadcast the reverse RREQ
//     SocketIpTtlTag tag;
//     p->RemovePacketTag(tag);
//     if (tag.GetTtl() < 2)
//     {
//         NS_LOG_DEBUG("TTL exceeded. Drop Rev_RREQ from origin " << src << " to destination " << dst);
//         return;
//     }
...
//         if (iface.GetMask() == Ipv4Mask::GetOnes())
//         {
//             destination = Ipv4Address("255.255.255.255");  // Broadcast address
//         }
//         else
//         {
//             destination = iface.GetBroadcast();  // Broadcast on the network
//         }
//         m_lastBcastTime = Simulator::Now();
//         Simulator::Schedule(Time(MilliSeconds(m_uniformRandomVariable->GetInteger(0, 10))),
//                             &RoutingProtocol::SendTo,
...
void RoutingProtocol::RecvRevRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
    NS_LOG_FUNCTION(this);

    // Remove the reverse RREQ header from the packet
    Rev_RreqHeader revRreqHeader;
    p->RemoveHeader(revRreqHeader);

    // Check if the node has received a reverse RREQ from a node in its blacklist
    RoutingTableEntry toPrev;
    ...
    // Check for duplicate reverse RREQ based on the ID and origin
    uint32_t id = revRreqHeader.GetId();
    Ipv4Address origin = revRreqHeader.GetOrigin();
    if (!IsMyOwnAddress(revRreqHeader.GetOrigin()) && m_rreqIdCache.IsDuplicate(origin, id))
    {
        NS_LOG_DEBUG("Ignoring Rev_RREQ due to duplicate");
        return;
    }

    // Increment the Hop Count for the reverse RREQ
    uint8_t hop = revRreqHeader.GetHopCount() + 1;
    revRreqHeader.SetHopCount(hop);

    // Look up the route for the destination (origin of the reverse request)
    RoutingTableEntry toDst;
    if (!m_routingTable.LookupRoute(revRreqHeader.GetDst(), toDst))
    {
        // If no route exists, create a new reverse route
        Ptr<NetDevice> dev = m_ipv4->GetNetDevice(m_ipv4->GetInterfaceForAddress(receiver));
        RoutingTableEntry newEntry(dev, revRreqHeader.GetDst());
        newEntry.SetValidSeqNo(true);
        newEntry.SetSeqNo(revRreqHeader.GetOriginSeqno());
        newEntry.SetInterface(m_ipv4->GetAddress(m_ipv4->GetInterfaceForAddress(receiver), 0));
        newEntry.SetHop(hop);
        newEntry.SetNextHop(src);
        newEntry.SetLifeTime(Time(2 * m_netTraversalTime - 2 * hop * m_nodeTraversalTime));
        m_routingTable.AddRoute(newEntry);
    }
    else
    {
        // Update the route with the new sequence number and other parameters
        if (toDst.GetValidSeqNo() && int32_t(revRreqHeader.GetOriginSeqno()) - int32_t(toDst.GetSeqNo()) > 0)
        {
            toDst.SetSeqNo(revRreqHeader.GetOriginSeqno());
        }
        toDst.SetValidSeqNo(true);
        toDst.SetNextHop(src);
        toDst.SetOutputDevice(m_ipv4->GetNetDevice(m_ipv4->GetInterfaceForAddress(receiver)));
        toDst.SetInterface(m_ipv4->GetAddress(m_ipv4->GetInterfaceForAddress(receiver), 0));
        toDst.SetHop(hop);
        toDst.SetLifeTime(std::max(Time(2 * m_netTraversalTime - 2 * hop * m_nodeTraversalTime), toDst.GetLifeTime()));
        m_routingTable.Update(toDst);
    }

    // If the receiver is the destination, send a reverse reply (RREP)
    if (IsMyOwnAddress(revRreqHeader.GetOrigin()))
    {
        NS_LOG_DEBUG("Send reverse reply since I am the destination");
        m_routingTable.LookupRoute(revRreqHeader.GetDst(), toDst);
        SendPacketFromQueue(revRreqHeader.GetDst(), toDst.GetRoute());
        return;
    }

    // Update the routing table and insert precursor information
    RoutingTableEntry toOrigin;
    if (!m_routingTable.LookupRoute(revRreqHeader.GetDst(), toOrigin) || toOrigin.GetFlag() == IN_SEARCH)
    {
        return; // Impossible, drop the packet
    }
    ...
    // Check if the destination has a valid route
    ...
    // Check TTL and forward the reverse RREQ if TTL is valid
    SocketIpTtlTag tag;
    p->RemovePacketTag(tag);
    if (tag.GetTtl() < 2)
    {
        NS_LOG_DEBUG("TTL exceeded. Drop Rev_RREQ from origin " << src << " to destination " << revRreqHeader.GetDst());
        return;
    }

    // Broadcast the reverse RREQ to all available interfaces
    for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
    {
        Ptr<Socket> socket = j->first;
        Ipv4InterfaceAddress iface = j->second;
