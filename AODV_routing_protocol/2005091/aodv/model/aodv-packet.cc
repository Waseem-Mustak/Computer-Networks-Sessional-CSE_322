...
// RRREQ  // new by zisan
//-----------------------------------------------------------------------------
Rev_RreqHeader::Rev_RreqHeader(uint8_t flags,
                       uint8_t reserved,
                       uint8_t hopCount,
                       uint32_t requestID,
                       Ipv4Address dst,
...
NS_OBJECT_ENSURE_REGISTERED(Rev_RreqHeader);

TypeId
Rev_RreqHeader::GetTypeId()
{
    static TypeId tid = TypeId("ns3::aodv::Rev_RreqHeader")
                            .SetParent<Header>()
                            .SetGroupName("Aodv")
                            .AddConstructor<Rev_RreqHeader>();
    return tid;
}

TypeId
Rev_RreqHeader::GetInstanceTypeId() const
{
    return GetTypeId();
}

uint32_t
Rev_RreqHeader::GetSerializedSize() const
{
    return 23;
}

void
Rev_RreqHeader::Serialize(Buffer::Iterator i) const
{
    i.WriteU8(m_flags);
    i.WriteU8(m_reserved);
    i.WriteU8(m_hopCount);
...
uint32_t
Rev_RreqHeader::Deserialize(Buffer::Iterator start)
{
    Buffer::Iterator i = start;
    m_flags = i.ReadU8();
    m_reserved = i.ReadU8();
...
void
Rev_RreqHeader::Print(std::ostream& os) const
{
    os << "RRREQ ID " << m_requestID << " destination: ipv4 " << m_dst << " sequence number "
       << m_dstSeqNo << " source: ipv4 " << m_origin << " sequence number " << m_originSeqNo
       << " flags:"
       << " Gratuitous RREP " << (*this).GetGratuitousRrep() << " Destination only "
       << (*this).GetDestinationOnly() << " Unknown sequence number " << (*this).GetUnknownSeqno();
...
std::ostream&
operator<<(std::ostream& os, const Rev_RreqHeader& h)
{
    h.Print(os);
    return os;
}
...
void
Rev_RreqHeader::SetGratuitousRrep(bool f)
{
    if (f)
    {
        m_flags |= (1 << 5);
...
bool
Rev_RreqHeader::GetGratuitousRrep() const
{
    return (m_flags & (1 << 5));
}

void
Rev_RreqHeader::SetDestinationOnly(bool f)
{
    if (f)
    {
        m_flags |= (1 << 4);
...
bool
Rev_RreqHeader::GetDestinationOnly() const
{
    return (m_flags & (1 << 4));
}

void
Rev_RreqHeader::SetUnknownSeqno(bool f)
{
    if (f)
    {
        m_flags |= (1 << 3);
...
bool
Rev_RreqHeader::GetUnknownSeqno() const
{
    return (m_flags & (1 << 3));
}

bool
Rev_RreqHeader::operator==(const Rev_RreqHeader& o) const
{
    return (m_flags == o.m_flags && m_reserved == o.m_reserved && m_hopCount == o.m_hopCount &&
            m_requestID == o.m_requestID && m_dst == o.m_dst && m_dstSeqNo == o.m_dstSeqNo &&
            m_origin == o.m_origin && m_originSeqNo == o.m_originSeqNo);
